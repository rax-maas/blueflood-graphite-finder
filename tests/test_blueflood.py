...
class TestBlueFlood(TestCase):
    def setUp(self):
        ...
        config = {'blueflood': {'tenant': self.dummy_tenant,
                                'urls': [self.dummy_bf_url]}}
    ...
    def test_find_metrics_endpoint(self):
        ...
        self.assertEquals(
            'localhost/v2.0/836986/metrics/search?include_enum_values=true',
            # noqa
    ...
    def test_find_nodes_all(self):
        prefix = '*'
        bf_mock_response = [{"metric": "one.two.three00.fourA.five100",
                             "enum_values": ["ev1-1", "ev2-1"]},
                            {"metric": "one.two.three00.fourD"},
                            {"metric": "one.two.three00.fourC.five0"},
                            {"metric": "one.two.three00.fourE",
                             "enum_values": ["ev1", "ev2"]},
                            {"metric": "one.two.three00.fourC.five2"},
                            {"metric": "one.two.three00",
                             "enum_values": ["fourA", "fourB"]},
                            {"metric": "one.two.three00.fourC.five1"},
                            {"metric": "foo1.bar2",
                             "enum_values": ["ev1", "ev2"]},
                            {"metric": "one.two.three00.fourA"},
                            {"metric": "one.two.three00.fourB.five2"},
                            {"metric": "one.two.three00.fourD.five100",
                             "enum_values": ["ev1-2", "ev2-2"]},
        ...
        self.verify_find_nodes_call(prefix, expected_branch_nodes,
                                    expected_leaf_nodes,
    ...
    def test_find_nodes_simple_prefix(self):
        ...
        bf_enum_mock_response = [{"metric": "one.two.three00.fourA.five100",
                                  "enum_values": ["ev1-1", "ev2-1"]},
                                 {"metric": "one.two.three00.fourD"},
                                 {"metric": "one.two.three00.fourC.five0"},
                                 {"metric": "one.two.three00.fourE",
    ...
    def test_find_nodes_partial_complete_metric_name(self):
        ...
        expected_branch_nodes = \
            {'one.two.three00.fourA',
             'one.two.three00.fourB',
             'one.two.three00.fourC',
             'one.two.three00.fourD',
             'one.two.three00.fourE'}  # fourE has enum values which makes it a branch node # noqa
        expected_leaf_nodes = \
            {'one.two.three00.fourA',
             'one.two.three00.fourD',
             'one.two.three00.fourB'}  # metric one.two.three00 which has fourA, fourB as enum values # noqa
    ...
    def test_find_nodes_enum_as_next_level(self):
        ...
        # True indicates leaf node
        expected_branch_nodes = {
            'one.two.three00.fourA.five100'}  # It is branch node because it has enum values # noqa
    ...
    def verify_find_nodes_call(self, prefix, expected_branch_nodes,
        ...
        with requests_mock.mock() as m:
            ...
            if len(query_parts) > 1:
                ...
                m.get(
                    '/v2.0/99999/metrics/search?include_enum_values=true&query=' + enum_name,  # noqa
